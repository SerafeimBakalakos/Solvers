...
namespace MGroup.Solvers.LinearAlgebraExtensions.Matrices.Builders
{
	using System;
	...
	/// </summary>
	public class DokSymmetric_v2 : ISparseSymmetricMatrix, ISymmetricMatrixBuilder
	{
		/// <summary>
		...
		private DokSymmetric_v2(int order, Dictionary<int, double>[] columns)
		{
		...
		public static DokSymmetric_v2 CreateEmpty(int order)
		{
			...
			return new DokSymmetric_v2(order, columns);
		}

		/// <summary>
		/// Initializes a new instance of <see cref="DokSymmetric_v2"/> with the specified matrix dimensions and entries 
		...
		/// </summary>
		/// <param name="order">The number of rows/columns of the symmetric matrix to build.</param>
		public static DokSymmetric_v2 CreateIdentity(int order)
		{
			var columns = new Dictionary<int, double>[order];
		...
		/// <summary>
		/// Initializes a new instance of <see cref="DokSymmetric_v2"/> with the specified matrix dimensions and the 
		...
		///     new <see cref="DokSymmetric_v2"/>. Its symmetry will not be checked explicitly.</param>
		public static DokSymmetric_v2 CreateFromSparseMatrix(ISparseMatrix matrix)
		{
		...
		public static DokSymmetric_v2 CreateFromSparsePattern(int order,
			IEnumerable<(int row, int col, double value)> nonZeroEntries)
		{
			DokSymmetric_v2 dok = CreateEmpty(order);
		...
		/// </summary>
		/// <param name="matrix">A symmetric sparse matrix whose dimensions and non-zero entries will be used to intialize the 
		///     new <see cref="DokSymmetric_v2"/>.</param>
		public static DokSymmetric_v2 CreateFromSparseSymmetricMatrix(ISparseSymmetricMatrix matrix)
		{
		...
		public static DokSymmetric_v2 CreateFromSparseSymmetricPattern(int order,
			IEnumerable<(int row, int col, double value)> nonZeroEntries)
		{
			DokSymmetric_v2 dok = CreateEmpty(order);
		...
		public void AddSubmatrixSymmetric(IIndexable2D submatrix, int[] submatrixToGlobalIndices)
		{
			int order = submatrix.NumColumns;
			Debug.Assert(submatrix.NumRows == submatrix.NumColumns);
			Debug.Assert(submatrixToGlobalIndices.Length == order);

			for (int j = 0; j < order; ++j)
			{
				int globalCol = submatrixToGlobalIndices[j];
				for (int i = 0; i < order; ++i)
				{
					int globalRow = submatrixToGlobalIndices[i];
					if (globalRow <= globalCol)
					{
						double subValue = submatrix[i, j];
						if (columns[globalCol].TryGetValue(globalRow, out double oldGlobalValue))
						{
							columns[globalCol][globalRow] = subValue + oldGlobalValue;
						}
		...
		/// This method should be called after fully defining the matrix in <see cref="DokSymmetric_v2"/> format.
		/// </summary>
		...
		/// called after fully defining the matrix in <see cref="DokSymmetric_v2"/> format.
		/// </summary>
		/// <param name="sortColsOfEachCol">True to sort the column indices of the CSC matrix between colOffsets[j] and 
		///     colOffsets[j+1] in ascending order. False to leave them unordered. Ordered rows might result in better  
		///     performance during multiplications or they might be required by 3rd party libraries. Conversely, leaving them 
		...
		/// <summary>
		/// Frees all memory held by this <see cref="DokSymmetric_v2"/> instance. Afterwards this object cannot be reused. 
		...
		/// <summary>
		/// Creates a new <see cref="DokSymmetric_v2"/> by copying only the entries whose row and column index belongs to 
		...
		/// </param>
		public DokSymmetric_v2 GetSubmatrix(int[] rowsColsToKeep)
		{
