namespace MGroup.Solvers.MachineLearning.PodAmg.Surrogates
{
	...
	public class CaeFfnnFullHistorySurrogateDynamicPythonTF : ISolutionPredictionStrategy
	{
		...
		private Dictionary<int, Vector> initialGuessesForFullHistory = new Dictionary<int, Vector>();
		private int numTimeSteps;

		public CaeFfnnFullHistorySurrogateDynamicPythonTF(CaeFfnnArchitecture caeFfnnArchitecture, string workDirectory, int pythonModelID)
		{
		...
		/// </summary>
		public bool CleanupIOFiles { get; set; } = false;

		public bool Float64 { get; set; } = false;

		public INormalizationStrategy NormalizationOfParameters { get; set; } = new NullNormalization();

		public INormalizationStrategy NormalizationOfSolutions { get; set; } = new NullNormalization();

		public bool ReadMLNetworksFromFilesWithoutTraining { get; set; } = true;
		...
		public void SetPythonCodePaths(string pythonInterpreter, string trainScript, string predictScript)
		{
			...
			this.predictHistoryScript = predictScript;
		}
		...
		public double[] Predict(int timeStep, double[] parameters)
		{
			if (timeStep == 0)
			{
				PredictFullHistory(parameters);
			}

			return initialGuessesForFullHistory[timeStep].RawData;
		}

		private void PredictFullHistory(double[] parameters)
		{
		...
		public void Train(SolutionDatabaseDynamic solutionDb)
		{
			...
			watch.Start();
			float[,] allParams = solutionDb.ToFloatArray2DAllParametersAndTimestepsAsRows(includeTimestep: false, true); // needs adjusting if the timestep is not included
