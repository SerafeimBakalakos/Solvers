namespace MGroup.Solvers.Multigrid.LinearAlgebraExtensions.Implementations.Managed
{
	using System;
	...
	public class ManagedStationaryIterationProvider
	{
		...
		public void CsrJacobiWeighted(int matrixOrder, double[] csrValues, int[] csrRowOffsets, int[] csrColIndices,
			int[] diagOffsets, double[] rhs, double[] solution, double omega, double[] work)
		{
			...
			for (int i = 0; i < matrixOrder; ++i)
			{
				...
				solution[i] = oneMinusOmega * work[i] + omega * sum / diagEntry;
			}
		}
